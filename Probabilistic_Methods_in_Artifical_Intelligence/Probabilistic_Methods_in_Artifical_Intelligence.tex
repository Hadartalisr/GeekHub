...
\begin{algorithm}[H]
    \SetNoFillComment
    \SetAlgoLined
    \KwIn{An oracle that returns $\forall X,Y,Z \subseteq \mathcal{X}$ if $P \models X \perp Y | Z$} 
    \KwOut{$\mathcal{H}$ - a minimal I-map of $P$}
    \ForEach{$X_i \in {X_1, \ldots, X_n}$}{
        \ForEach{$X_j \in \{ X_1, \ldots, X_n \} / \{X_i\} $}{
            \If{$P \not\models X_i \perp X_j | \mathcal{X} / \{X_i, X_j\}$}{
                $\mathcal{H}_e \leftarrow \mathcal{H}_e \cup \{ X_i - X_j \}$
            }
        }
    }

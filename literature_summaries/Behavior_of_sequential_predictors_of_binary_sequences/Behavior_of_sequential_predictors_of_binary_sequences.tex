...
Then, utilizing (\ref{eq:3.6}) (as we did in \ref{eq:3.11})
\begin{equation*}
    \begin{aligned}
        \hat{p}_k(i) &= \frac{1}{2} + n b_k(i) \\
        &= \frac{1}{2} + n \left(\frac{1}{2}\right)^{n-k+1} \sum_{j=0}^{n-k} \left(\hat{s}(i+j+1) - \hat{s}(i+j)\right) \binom{n-k}{j}
    \end{aligned}
\end{equation*}
...
$\bar{s}(\Theta) = \frac{1}{2}$ for all $\Theta \in \{0, 1\}^n$ is achievable by the predictor ${p} = (\frac{1}{2}, \frac{1}{2}, \ldots, \frac{1}{2})$.
...
Given $\hat{s}$, the predictor $\hat{p}_k$ for each stage $k$ based on the history of sums can be designed as:
\[
\hat{p}_k(i) = \frac{1}{2} + n \left(\frac{1}{2}\right)^{n-k+1} \sum_{j=0}^{n-k} \left(\hat{s}(i+j+1) - \hat{s}(i+j)\right) \binom{n-k}{j}
\]
simplifies to $\hat{p}_k(i) = \frac{1}{2} + n \left(\frac{1}{2}\right)^{n-k+1} \frac{1}{n} \left(\binom{n-k}{0} - \binom{n-k}{n-k}\right) = \frac{1}{2}$ as all terms cancel out except the boundary cases, which simplifies back to $\frac{1}{2}$, making it a uniform predictor that always bets $\frac{1}{2}$ regardless of the history, consistent with the uniform distribution over binary outcomes.
